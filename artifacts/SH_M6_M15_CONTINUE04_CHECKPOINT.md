...
| 7 | §9 `/api/v1/documents` blueprint collision | **IMPLEMENTED (analysis only)** | Quantified with hard evidence: exactly ONE colliding rule, `GET /api/v1/documents`, owned by `document_intel.api_list` (RBAC, `knowledge.view`) vs `documents.list_documents` (identity-header only) — resolved silently by registration order. A THIRD surface `/api/v1/workspace/documents` (`documents_api`) is the real frontend contract. Guard test drafted | Collision NOT de-duplicated; guard NOT committed |
| 8 | §5 empty states — dead ends | **NOT STARTED (finding recorded)** | `document-browser.tsx` empty state instructs 'Use "Add to My SHUNYA" to upload files' — **that affordance exists nowhere in the codebase**, and the component has no upload action. A literal dead end AND misleading | needs a real continuation wired via `useWorkspaceStore.getState().open(label,'import-export')` |
