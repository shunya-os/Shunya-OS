  ...
  return (
    <div style={{ position: 'fixed', bottom: 8, right: 8, zIndex: 9999, fontFamily: 'monospace', fontSize: 11 }}>
      <button onClick={() => setOpen(!open)} style={{
        background: data?.health.failed ? '#9B2226' : '#2D6A4F', color: '#fff', border: 'none',
      ...
      {open && data && (
        <div style={{
          position: 'absolute', bottom: 28, right: 0, width: 360, maxHeight: 400, overflow: 'auto',
          background: '#1A1A1E', color: '#E8E3DA', borderRadius: 8, padding: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.4)',
        }}>
          <div style={{ fontWeight: 600, marginBottom: 8, fontSize: 13 }}>Runtime Orchestrator</div>
          <div style={{ marginBottom: 6 }}>Status: {data.health.ready}/{data.health.total} ready
            {data.health.failed ? ` · ${data.health.failed} failed` : ''}
          </div>
          <div style={{ marginBottom: 8, whiteSpace: 'pre-wrap', fontSize: 10, opacity: 0.7 }}>Startup order:
{data.chain}</div>
