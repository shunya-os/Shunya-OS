  ...
  return {
    ...
    open: (name, type, opts) => {
      ...
      const existing = state.workspaces.find(
        (w) =>
          w.identity.objectType === opts?.objectType &&
          w.identity.objectId === opts?.objectId &&
          w.identity.type === type,
      );
      if (existing) {
        get().activate(existing.identity.id);
    ...
    close: (id) => {
      ...
      bus.emit({
        type: 'ObjectClosed',
        objectType: ws.identity.objectType ?? '',
        objectId: ws.identity.objectId ?? '',
        workspaceId: id,
      });
      ...
      set((s) => ({
        ...
        activeId:
          s.activeId === id ? (s.workspaces.find((w) => w.identity.id !== id)?.identity.id ?? null) : s.activeId,
      }));
      get().prune();
      get().persist();
    },
    ...
    activate: (id) => {
      ...
      set((s) => ({ ...s, activeId: id }));
      // Push URL state for proper back/forward navigation — skip if URL already matches
      const objectId = ws.identity.objectId;
      if (objectId && !objectId.startsWith('_')) {
        const targetUrl = `/workspace/${objectId}`;
        if (window.location.pathname !== targetUrl) {
          window.history.pushState({ workspaceId: objectId }, '', targetUrl);
        }
      }
    },
    ...
    markDirty: (id, dirty) => {
      set((s) => ({
        ...s,
        workspaces: s.workspaces.map((w) => (w.identity.id === id ? { ...w, dirty } : w)),
      }));
      if (dirty) {
        const ws = get().workspaces.find((w) => w.identity.id === id);
        if (ws) {
          bus.emit({
            type: 'ObjectDirtyChanged',
            objectType: ws.identity.objectType ?? '',
            objectId: ws.identity.objectId ?? '',
            dirty,
          });
    ...
    markSaving: (id) => {
      set((s) => ({
        ...s,
        workspaces: s.workspaces.map((w) => (w.identity.id === id ? { ...w, saveStatus: 'saving' as const } : w)),
      }));
    },
    ...
    setValidationErrors: (id, errors) => {
      set((s) => ({
        ...s,
        workspaces: s.workspaces.map((w) => (w.identity.id === id ? { ...w, validationErrors: errors } : w)),
      }));
    },
    ...
    prune: () => {
      ...
      set((s) => ({
        ...s,
        workspaces: s.workspaces.filter((w) => w.identity.pinned || w.identity.lastAccessed > cutoff),
      }));
    },

    persist: () => {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(get().workspaces));
      } catch {
        /* noop */
      }
    },
