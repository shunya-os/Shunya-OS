...
export const CompositionEngine = {
  compose(
    ...
    runtimeStates: Record<string, Record<string, unknown>>,
  ): ComposedWorkspace {
    ...
    const panels: ComposedPanel[] = panelSlots.map((slot) => {
      ...
      if (!comp)
        return {
          ...
          props: {},
          loading: true,
          error: `No composition for ${slot.id}`,
        };
      ...
      if (!meta)
        return {
          ...
          props: {},
          loading: true,
          error: `Unknown component: ${comp.componentId}`,
        };
      ...
      if (failed) {
        return {
          id: slot.id,
          componentId: comp.componentId,
          Component: meta.component as ComponentType<any>,
          props: {},
          loading: false,
          error: `${comp.label || comp.componentId} unavailable`,
          label: comp.label,
        };
      }

      const state = comp.dependsOn.reduce(
        (acc, dep) => Object.assign(acc, runtimeStates[dep] ?? {}),
