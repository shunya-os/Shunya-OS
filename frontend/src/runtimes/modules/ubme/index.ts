/**
 * UBME Module — Universal Business Model Engine.
 *
 * This module provides:
 * 1. Module Builder — admin UI for creating/editing business modules
 * 2. Universal Object Viewer — dynamically render any object type
 * 3. Dynamic Navigation — generated from installed modules
 *
...
// ── Dynamic styles injected on registration ──
...
function injectStyles() {
  if (document.getElementById('ubme-styles')) return;
  const style = document.createElement('style');
  style.id = 'ubme-styles';
...
const module: ShunyaModule = {
  id: 'ubme',
  name: 'Universal Business Model Engine',
  ...
  register: async (data: Record<string, any>) => {
      ...
      for (const entry of info.entries || []) {
        ...
        if (otKey) {
          WorkspaceRegistry.register({
            id: `ubme_${modKey}_${otKey}`,
            name: `${info.name} - ${entry.label}`,
            description: `View ${entry.label}`,
            supportedObjectTypes: [otKey],
            requiredRuntimes: [],
            layoutTemplate: 'list',
            panels: [],
          });
    ...
    // Also register a Module Builder workspace
    WorkspaceRegistry.register({
      id: 'ubme_builder',
      name: 'Module Builder',
      description: 'Create and manage business modules',
    ...
    for (const [modKey, modData] of Object.entries(nav)) {
      ...
      for (const entry of info.entries || []) {
        ...
        if (otKey) {
          WorkspaceRegistry.register({
            id: `ubme_${modKey}_${otKey}_workspace`,
            name: `${info.name} - ${entry.label} Workspace`,
            description: `Full workspace for ${entry.label}`,
            supportedObjectTypes: [otKey],
            requiredRuntimes: [],
            layoutTemplate: 'tabs',
            panels: [],
        ...
        for (const ot of mod.object_types || []) {
          try {
            ...
            for (const item of items) {
              const name = item.name || item.data?.name || item.id;
              if (name.toLowerCase().includes(query.toLowerCase())) {
                results.push({
                  id: item.id,
                  type: ot.name,
                  title: name,
                  subtitle: `${mod.name} · ${ot.name}`,
                  status: item.status,
                });
              }
            }
...
// Consumed by the platform command palette system.
...
export const commandPaletteActions: CommandPaletteAction[] = [
  {
    id: 'ubme_open_builder',
    label: 'Open Module Builder',
    icon: '📦',
    category: 'UBME',
    action: () => {
      // The platform's command palette hook calls open with workspace id
      // This is consumed by the command palette system
      const evt = new CustomEvent('ubme:open-workspace', {
        detail: { workspaceId: 'ubme_builder' },
      });
  ...
  },
  {
    ...
    action: () => {
      const evt = new CustomEvent('ubme:open-workspace', {
        detail: { workspaceId: 'ubme_builder', action: 'new-module' },
      });
      document.dispatchEvent(evt);
    },
  },
