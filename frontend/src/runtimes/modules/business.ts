...
const module: ShunyaModule = {
  ...
  register: async (data: Record<string, any>) => {
      ...
      panels: [
        {
          ...
          propsResolver: () => ({
            value: customers,
            subtitle: customers > 0 ? 'Active customers' : 'No customers yet',
          }),
        },
        {
          componentId: 'metric',
        ...
        },
        {
          ...
          propsResolver: () => ({
            value: commitments,
            subtitle: commitments > 0 ? 'Active commitments' : 'No commitments',
          }),
        },
        {
          componentId: 'metric',
          ...
          propsResolver: () => ({
            value: conversations,
            subtitle: conversations > 0 ? 'Active conversations' : 'No conversations',
          }),
        },
        {
          ...
          ? [
              {
                ...
                propsResolver: () => {
                  const atRisk = activeCommitments.find((c: any) => {
                    try {
                      return JSON.parse(c.content ?? '{}').status === 'at_risk';
                    } catch {
                      return false;
                    }
                  });
                  return {
                    ...
                    reason: atRisk
                      ? `This commitment is at risk — review and update status`
                      : `Your workspace has ${objCount} objects across ${Object.keys(types).length} types`,
                    confidence: 'high' as const,
                  };
                },
              },
    ...
    WorkspaceRegistry.register({
      id: 'object',
      name: 'Object',
      description: 'Business object view',
      supportedObjectTypes: ['*'],
      requiredRuntimes: [],
      layoutTemplate: 'object',
      panels: [
        {
          componentId: 'object-identity',
          dependsOn: ['object'],
          label: 'Identity',
          propsResolver: (s: any) => ({
            name: s.name ?? 'Unknown',
            type: s.object_type ?? 'object',
    ...
    WorkspaceRegistry.register({
      id: 'conversation',
      name: 'Conversation',
      description: 'Business conversation context',
      supportedObjectTypes: ['conversation'],
      requiredRuntimes: [],
      layoutTemplate: 'conversation',
      panels: [
        {
          componentId: 'conversation-card',
          dependsOn: [],
          critical: true,
          label: 'Conversation',
          propsResolver: (s: any) => {
            try {
              ...
              return { title: s.name, intent: '', status: 'active', participants: [], objectCount: 0 };
            }
          },
        },
        {
          ...
          propsResolver: (s: any) => {
            try {
              ...
              return {
                title: 'AI Analysis',
                body: d.summary ?? 'No summary available',
                confidence: ai.confidence ?? 'medium',
                type: 'observation',
              };
            } catch {
              return { title: 'AI Analysis', body: 'No summary available', confidence: 'medium', type: 'observation' };
            }
          },
        },
      ],
    ...
    WorkspaceRegistry.register({
      ...
      supportedObjectTypes: ['commitment'],
      requiredRuntimes: [],
      layoutTemplate: 'commitment',
      panels: [
        {
          ...
          propsResolver: (s: any) => {
            try {
              const d = JSON.parse(s.content ?? '{}');
              return { value: d.progress ?? 0, label: 'Commitment progress' };
            } catch {
              return { value: 0, label: 'Commitment progress' };
            }
          },
        },
        {
          componentId: 'confidence-meter',
          dependsOn: [],
          label: 'Confidence',
          propsResolver: (s: any) => {
            try {
              const d = JSON.parse(s.content ?? '{}');
              return { score: d.confidence ?? 0, factors: d.confidence_factors ?? [] };
            } catch {
        ...
        {
          componentId: 'blocker-list',
          dependsOn: [],
          label: 'Blockers',
          propsResolver: (s: any) => {
            try {
              const d = JSON.parse(s.content ?? '{}');
              return { blockers: d.risks ?? [] };
            } catch {
              return { blockers: [] };
            }
          },
        },
        {
          ...
          propsResolver: (s: any) => {
            try {
              const d = JSON.parse(s.content ?? '{}');
              return {
                action: d.next_action ?? 'Review commitment',
                reason: `Progress: ${Math.round((d.progress ?? 0) * 100)}% · Confidence: ${Math.round((d.confidence ?? 0) * 100)}%`,
                confidence:
                  (d.confidence ?? 0) >= 0.7
                    ? ('high' as const)
                    : (d.confidence ?? 0) >= 0.4
              ...
              return { action: 'Review commitment', reason: 'No data available', confidence: 'low' as const };
            }
          },
        },
      ],
      ...
      if (total > 0) {
        ...
        try {
          ...
          const risky = objs.data.filter((o: any) => {
            try {
              const d = JSON.parse(o.content ?? '{}');
              return d.status === 'at_risk';
            } catch {
              return false;
            }
          });
