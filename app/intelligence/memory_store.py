...
def update_weight(key: str, delta: float) -> float:
    ...
    try:
        ...
        if entry:
            ...
            # Use PostgreSQL upsert for concurrent safety across processes.
            # If a concurrent process already inserted this key, ON CONFLICT
            # DO NOTHING silently succeeds. Falls back to regular add() for
