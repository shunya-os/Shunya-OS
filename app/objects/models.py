"""
SHUNYA OS — Phase 0 Foundation Models.

Workspace + Universal Object (ShunyaObject) + Object Type Registry.
All queries are scoped by workspace_id. All routes require X-Identity-Id.
"""
import uuid
...
def _generate_object_id() -> str:
    """Generate a unique object_id (UUID string)."""
    return str(uuid.uuid4())


# ── Lifecycle Configuration — EP-02-R1 ──
# Stage pipelines are defined here, not in the Composer.
# The Composer imports them; the Object Runtime owns them.
...
def get_stage_pipeline(object_type: str) -> list[str]:
    """Return the stage pipeline for the given object type."""
    return OBJECT_STAGE_PIPELINES.get(object_type, DEFAULT_STAGE_PIPELINE)


class Workspace(db.Model):
    __tablename__ = 'sh_workspaces'
    id = db.Column(db.String(20), primary_key=True)  # spc_ prefix
    name = db.Column(db.String(100), nullable=False)
    workspace_type = db.Column(db.String(20), nullable=False)  # business, personal, custom
...
class ShunyaObject(db.Model):
    ...
    object_id = db.Column(db.String(36), unique=True, nullable=False, default=_generate_object_id)  # UUID
    workspace_id = db.Column(db.String(20), db.ForeignKey('sh_workspaces.id'), nullable=False)
    object_type = db.Column(db.String(50), nullable=False)  # customer, invoice, proposal, task, etc.
    name = db.Column(db.String(500), nullable=False)
    status = db.Column(db.String(30), default='active')  # active, draft, archived, completed
    data = db.Column(db.JSON, default=dict)  # All type-specific fields stored as JSON
    created_by = db.Column(db.String(64), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
# Object Registry — defines what types exist and their fields
OBJECT_TYPES = {
    'customer': {
        'name': 'Customer',
...
def resolve_object_name(object_type: str, data: dict) -> str:
    """Resolve the display name for an object from its data payload."""
