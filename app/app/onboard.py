...
def onboard(identity_id: str) -> dict:
    ...
    with app.app_context():
        # Find the founder's organization — try identity_id first, then email
        member = OrgMember.query.filter_by(identity_id=identity_id).first()
        if not member:
            # Try to find by TeamMember email
            from app.auth import TeamMember
            tm = TeamMember.query.filter(
                TeamMember.email.isnot(None)
            ).first()
            if tm:
                member = OrgMember.query.filter_by(email=tm.email).first()
        if not member:
