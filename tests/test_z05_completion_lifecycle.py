...
Run directly (not collected by pytest as a unit test):
    python tests/test_z05_completion_lifecycle.py

This is a standalone end-to-end script. It creates its own app and
database, so it must NOT be collected as a pytest module (module-level
execution would pollute other tests' global app state).
"""

__test__ = False  # Do not collect this module as pytest tests
...
from app.auth import TeamMember  # noqa: E402
from app.tenant import Tenant  # noqa: E402
...
def _main():
    ...
    with app.app_context():
        ...
        record("AI chat responds 200", resp.status_code == 200, f"got {resp.status_code}")
        record("AI chat has content", bool(data.get("content")), "empty content")
        record("AI chat has command linkage", bool(data.get("command")), f"missing command block")
        if data.get("command"):
            cmd = data["command"]
            record("Command has outcome_id", bool(cmd.get("outcome_id")), str(cmd))
            record("Command has drilldown", bool(cmd.get("drilldown")), str(cmd))
        ...
        record("Question responds 200", resp2.status_code == 200)
        record("Question has no command linkage", not data2.get("command"), f"unexpected command block: {data2.get('command')}")
        ...
        record("Execution work endpoint responds 200", resp3.status_code == 200)
        record("Work has outcome from command", any(
        ...
        record("Execution outputs endpoint responds 200", resp3b.status_code == 200)
        ...
        record("Conversation created", bool(conv_id), str(conv_id))
        ...
        record("Message posted", resp4b.status_code == 201, str(resp4b.status_code))
        ...
        record("Refresh: conversation persists", resp4c.status_code == 200)
        record("Refresh: message persists", len(timeline) >= 1, f"got {len(timeline)} timeline entries")
        ...
        record("Intelligence ask responds 200", resp5.status_code == 200, f"got {resp5.status_code}")
        record("Intelligence has answer", bool(data5.get("answer")), "missing answer")
        ...
        record("Memory entries responds 200", resp6.status_code == 200)
        entries = data6.get("data", {}).get("entries", [])
        record("Memory has command entries from AI execution",
        ...
        record("Memory knowledge responds 200", resp6b.status_code == 200)
        ...
        record("Founder AI health responds 200", resp7.status_code == 200, f"got {resp7.status_code}")
        record("Founder AI is healthy", data7.get("data", {}).get("status") == "healthy", str(data7))
