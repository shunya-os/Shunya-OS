...
class TestImportPreview:
    ...
    CSV_VALID = "customer_name,phone,email\nTest User,+911****7890,test@example.com\nSecond User,+919****3210,second@example.com"
    ...
    def _login(self, client):
        ...
        db.session.commit()
        org_id = seed_rbac(db, identity_id="sid_import_preview", role_name="admin")
...
class TestImportCommit:
    ...
    def _login(self, client):
        ...
        db.session.add(member)
        db.session.commit()
        org_id = seed_rbac(db, identity_id="sid_import_test", role_name="admin")
        with client.session_transaction() as sess:
            sess["user_id"] = member.id
